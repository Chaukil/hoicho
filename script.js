        ...
        paginatedProducts.forEach(product => {
            ...
            <tr>
                ...
                <td class="current-stock-value">
                    <strong class="${currentStock <= 0 ? 'text-danger' :
                    currentStock <= product.minStock ? 'text-warning' :
                        'text-success'}">${currentStock}</strong>
                </td>
...
function editSold(soldId) {
    ...
    soldItem.products.forEach(product => {
        ...
        // Đặt giá trị số lượng TRƯỚC KHI cập nhật thông tin tồn kho
        quantityInput.value = product.quantity;

        // Cập nhật thông tin tồn kho KHÔNG reset input
        setTimeout(() => {
            updateStockInfoForEdit(select, product.quantity);
        }, 100);
    });
...
// Function mới để cập nhật thông tin tồn kho khi sửa (không reset input)
function updateStockInfoForEdit(selectElement, currentQuantity) {
    ...
    if (productId) {
        ...
        // KHÔNG reset giá trị input khi sửa, chỉ cập nhật max và trạng thái
        quantityInput.setAttribute('max', currentStock + currentQuantity); // Cho phép số lượng hiện tại + tồn kho

        if (currentStock <= 0 && currentQuantity === 0) {
            quantityInput.disabled = true;
            quantityInput.placeholder = 'Hết hàng';
        } else {
            quantityInput.disabled = false;
            quantityInput.placeholder = `Max: ${currentStock + currentQuantity}`;
        }
    } else {
...
function updateStockInfo(selectElement) {
    ...
    if (productId) {
        ...
        // CHỈ reset input khi đây là thêm mới (không có giá trị từ trước)
        if (!quantityInput.dataset.isEditing) {
            quantityInput.value = '';
        }
        ...
        if (currentStock <= 0) {
            ...
            quantityInput.disabled = false;
            quantityInput.placeholder = `Tối đa: ${currentStock}`;
        }
    } else {
        // Reset khi không chọn sản phẩm
        stockDisplay.innerHTML = '<span class="badge bg-secondary">-</span>';
...
function validateQuantity(inputElement) {
    ...
    if (productId && quantity) {
        ...
        // Lấy số lượng ban đầu nếu đang sửa
        const originalQuantity = parseInt(inputElement.dataset.originalQuantity || 0);
